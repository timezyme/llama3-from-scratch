...
|------------|---------------|
| KV cache | `include/kv_cache.h`, `src/kv_cache.cu`, and `src/inference.cu` can store K/V rows during multi-token generation. Correctness was checked earlier, but the performance gate still needs the L4 machine. |
| Multi-token loop | `main.cpp` accepts `--max-tokens N` and calls `generate_tokens`. |
| Telemetry | `include/instrument.h` prints timing and VRAM checkpoints. |
| BF16-weight matmul primitive | `gpu_matmul_device_bf16_weight` stores weights in BF16 form on device and expands them inside the kernel. This is a stepping stone toward resident BF16 weights and tensor cores on L4. |

Do not read that table as "all bonus credit is done." It is the work-in-progress path toward the bonus items.
